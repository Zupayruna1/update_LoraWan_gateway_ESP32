...
#	define ESP32_ARCH 1
#	ifndef _PIN_OUT
#		define _PIN_OUT 5									// For ESP32 pin-out 4 is default
#	endif
...
sf_t sf2 			= _SPREADING2;							// Initial value of SF					
...
void initLoraModem2();													// _loraModem.ino
void rxLoraModem2();														// _loraModem.ino
void writeRegister2(uint8_t addr, uint8_t value);						// _loraModem.ino
void cadScanner2();														// _loraModem.ino
void startReceiver2();													// _loraModem.ino

void stateMachine2();													// _stateMachine.ino
//**********************************************************************************************************
...
#	endif //_UDPROUTER
	...
	pinMode(pins.ss2, OUTPUT);
	pinMode(pins.rst2, OUTPUT);
    pinMode(pins.dio02, INPUT);								// This pin is interrupt
	pinMode(pins.dio12, INPUT);								// This pin is interrupt
...
#	if _MONITOR>=1
	if (debug>=0) {
		...
		response += ", Listening at SF" + String(sf2) + " on ";
		response += String(freqs[gwayConfig.ch2].upFreq) + " Hz.";
		mPrint(response);
	}
#	endif //_MONITOR
...
#		endif
	...
	if (gwayConfig.cad) {
		_state2 = S_SCAN;
		sf = SF7;
		cadScanner2();										// Always start at SF7
	}
	else { 
		_state2 = S_RX;
		rxLoraModem2();
	...
	if (pins.dio02 == pins.dio12) {
		attachInterrupt(pins.dio02, Interrupt_02, RISING);	// Share interrupts
	}
	// Or in the traditional Comresult case
	else {
		attachInterrupt(pins.dio02, Interrupt_02, RISING);	// Separate interrupts
		attachInterrupt(pins.dio12, Interrupt_12, RISING);	// Separate interrupts
		//attachInterrupt(pins.dio2, Interrupt_2, RISING);	// Separate interrupts
...
#	endif //_OLED
	...
	writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);	// Allow all
