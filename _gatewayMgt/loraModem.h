...
volatile state_t _state2=S_INIT;
volatile uint8_t _event2=0;
...
uint32_t msgTime2=0;							// in seconds, Thru nowSeconds, now()
uint32_t hopTime2=0;							// in micros()
uint32_t detTime2=0;							// In micros()
...
// RST  == GPIO14/ PIN14
struct pins {
	uint8_t dio0=39;		// GPIO26 / Dio0 used for one frequency and one SF
	uint8_t dio1=39;		// GPIO35 / Used for CAD, may or not be shared with DIO0
	uint8_t dio2=39;		// GPIO34 / Used for frequency hopping, don't care
	uint8_t ss=23;			// GPIO18 / Dx. Select pin connected to GPIO18
	uint8_t rst=13;			// GPIO0 / D3. Reset pin not used	

	uint8_t dio02=34;		// GPIO26 / Dio0 used for one frequency and one SF
	uint8_t dio12=34;		// GPIO35 / Used for CAD, may or not be shared with DIO0
	uint8_t dio22=34;		// GPIO34 / Used for frequency hopping, don't care
	uint8_t ss2=32;			// GPIO18 / Dx. Select pin connected to GPIO18
	uint8_t rst2=15;			// GPIO0 / D3. Reset pin not used	
} pins;
#define SCK 18				// Check
#define MISO 36				// Check
#define MOSI 5				// Check
#define RST 13				// ------
#define RST2 15				// Check
#define SS 23
#define SS2 32              // -----
