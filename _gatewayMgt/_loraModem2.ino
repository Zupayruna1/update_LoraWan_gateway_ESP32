...
// in interrupt do access the readRegister2 and writeRegister2() function
...
void writeRegister2(uint8_t addr, uint8_t value)
{
...
void writeBuffer2(uint8_t addr, uint8_t *buf, uint8_t len)
{
	//noInterrupts();								// MMM

	SPI.beginTransaction(writeSettings);
	digitalWrite(pins.ss2, LOW);					// put SPI slave on
    ...
    digitalWrite(pins.ss2, HIGH);				// Unselect SPI slave
...
// ----------------------------------------------------------------------------------------
//  setRate2 is setting rate and spreading factor and CRC etc. for transmission
//  for example
...
void setRate2(uint8_t sf, uint8_t crc) 
{
...
#		endif //_MONITOR
    ...
    //    writeRegister2(REG_PAYLOAD_LENGTH, getIh(LMIC.rps)); // required length
    //}
	
	writeRegister2(REG_MODEM_CONFIG1, (uint8_t) mc1);
	writeRegister2(REG_MODEM_CONFIG2, (uint8_t) mc2);
	writeRegister2(REG_MODEM_CONFIG3, (uint8_t) mc3);
	
	// Symbol timeout settings
    if (sf == SF10 || sf == SF11 || sf == SF12) {
        writeRegister2(REG_SYMB_TIMEOUT_LSB, (uint8_t) 0x05);
    } 
	else {
        writeRegister2(REG_SYMB_TIMEOUT_LSB, (uint8_t) 0x08);
    }
	return;
}
...
void  setFreq2(uint32_t freq)
{
    ...
    writeRegister2(REG_FRF_MSB, ((uint8_t)(frf>>16)) & 0xFF );
    writeRegister2(REG_FRF_MID, ((uint8_t)(frf>> 8)) & 0xFF );
    writeRegister2(REG_FRF_LSB, ((uint8_t)(frf>> 0)) & 0xFF );
	return;
}
...
void setPow2(uint8_t pow)
{
	...
	writeRegister2(REG_PAC, (uint8_t) pac);								// set register 0x09 to pac
	return;
}
...
// ----------------------------------------------------------------------------------------
// Set the opmode2 to a value as defined on top
// Values are 0x00 to 0x07
// The value is set for the lowest 3 bits, the other bits are as before.
// ----------------------------------------------------------------------------------------
void opmode2(uint8_t mode)
{
	if (mode == OPMODE_LORA) {
#		ifdef CFG_sx1276_radio
		//mode |= 0x08;   											// TBD: sx1276 high freq
#		endif
		writeRegister2(REG_OPMODE, 0xFF & (uint8_t) mode );
	}
	else {
		// If in OPMODE_LORA, leave it is (0x80)
		writeRegister2(REG_OPMODE, 0xFF & (uint8_t)((readRegister2(REG_OPMODE) & 0x80) | mode));
	}
}
...
void hop2() 
{
	...
	opmode2(OPMODE_STANDBY);
		
	// 3. Set frequency based on value in freq		
	gwayConfig.ch2 = (gwayConfig.ch2 + 1) % NUM_HOPS ;			// Increment the freq round robin
	setFreq2(freqs[gwayConfig.ch2].upFreq);
	
	// 4. Set spreading Factor
	sf = SF7;													// Starting the new frequency 
	setRate2(sf, 0x04);											// set the sf to SF7, and CRC to 0x04
		
	// Low Noise Amplifier used in receiver
	writeRegister2(REG_LNA, (uint8_t) LNA_MAX_GAIN);  			// 0x0C, 0x23
	
	// 7. set sync word
	writeRegister2(REG_SYNC_WORD, (uint8_t) 0x34);				// set 0x39 to 0x34 LORA_MAC_PREAMBLE
	
	// prevent node to node communication
	writeRegister2(REG_INVERTIQ,0x27);							// set reg 0x33 to 0x27; to reset from TX
	...
	writeRegister2(REG_MAX_PAYLOAD_LENGTH,MAX_PAYLOAD_LENGTH);	// set 0x23 to 0x80==128 bytes
	writeRegister2(REG_PAYLOAD_LENGTH,PAYLOAD_LENGTH);			// 0x22, 0x40==64Byte long
	
	writeRegister2(REG_FIFO_ADDR_PTR,(uint8_t)readRegister2(REG_FIFO_RX_BASE_AD));	// set reg 0x0D to reg 0x0F(==0x00)
	writeRegister2(REG_HOP_PERIOD,0x00);							// reg 0x24, set to 0x00

	// 5. Config PA Ramp up time								// set reg 0x0A  
	writeRegister2(REG_PARAMP, (readRegister2(REG_PARAMP) & 0xF0) | 0x08); // set PA ramp-up time 50 uSec
	
	// Set 0x4D PADAC for SX1276 ; XXX register is 0x5a for sx1272
	writeRegister2(REG_PADAC_SX1276,  0x84); 					// set 0x4D (PADAC) to 0x84
	
	// 8. Reset interrupt Mask, enable all interrupts
	writeRegister2(REG_IRQ_FLAGS_MASK, 0x00);
		
	// 9. clear all radio IRQ flags
    writeRegister2(REG_IRQ_FLAGS, 0xFF);
	
	// Be aware that micros() has increased significantly from calling 
	// the hop2 function until printed below
	//
#	if _MONITOR>=1
	if ((debug>=2) && (pdebug & P_RADIO)){
			String response = "hop2:: hopTime2:: " + String(micros() - hopTime2);
			mStat(0, response);
			mPrint(response);
	}
#	endif //_MONITOR
	// Remember the last time we hop2
	hopTime2 = micros();									// At what time did we hop2
...
//
// ----------------------------------------------------------------------------------------
uint8_t receivePkt2(uint8_t *payload)
{
    statc.msg_ttl++;													// Receive statistics counter

    uint8_t irqflags = readRegister2(REG_IRQ_FLAGS);						// 0x12; read back flags											
	uint8_t crcUsed  = readRegister2(REG_HOP_CHANNEL);					// Is CRC used? (Register 0x1C)
	if (crcUsed & 0x40) {
#		if _DUSB>=1
...
#		endif //_MONITOR
        ...
        writeRegister2(REG_IRQ_FLAGS, (uint8_t)(IRQ_LORA_HEADER_MASK  | IRQ_LORA_RXDONE_MASK));	// 0x12; clear HEADER (== 0x10) flag
        return 0;
    }
	...
	// This means "Set FifoAddrPtr to FifoRxBaseAddr"
		...
		if (readRegister2(REG_FIFO_RX_CURRENT_ADDR) != readRegister2(REG_FIFO_RX_BASE_AD)) {
#			if _MONITOR>=1
			if ((debug>=1) && (pdebug & P_RADIO)) {
				mPrint("RX BASE <" + String(readRegister2(REG_FIFO_RX_BASE_AD)) + "> != RX CURRENT <" + String(readRegister2(REG_FIFO_RX_CURRENT_ADDR)) + ">"	);
			}
#			endif //_MONITOR
		}

        //uint8_t currentAddr = readRegister2(REG_FIFO_RX_CURRENT_ADDR);	// 0x10
		uint8_t currentAddr   = readRegister2(REG_FIFO_RX_BASE_AD);		// 0x0F
        uint8_t receivedCount = readRegister2(REG_RX_BYTES_NB);			// 0x13; How many bytes were read
#		if _MONITOR>=1
			if ((debug>=1) && (currentAddr > 64)) {						// More than 64 read?
				mPrint("rxPkt:: ERROR Rx addr>64"+String(currentAddr));
			}
#		endif //_MONITOR
        writeRegister2(REG_FIFO_ADDR_PTR, (uint8_t) currentAddr);		// 0x0D 
...
#			endif //_MONITOR
        ...
        {
            payload[i] = readRegister2(REG_FIFO);						// 0x00, FIFO will auto shift register
        }
...
#	if _MONITOR>=1
		if ((debug>=1) && (pdebug & P_RX)) {
			...
			String response = "^ receivePkt2:: rxPkt: t=";
			stringTime(now(), response);
			response += ", f=" + String(gwayConfig.ch2) + ", sf=" + String(sf);
...
#	endif //_MONITOR
	...
	writeRegister2(REG_IRQ_FLAGS, (uint8_t) (
...
// 
// NOTE:: writeRegister2 functions should not be used outside interrupts
...
//	Byte 6:
// ----------------------------------------------------------------------------------------
bool sendPkt2(uint8_t *payLoad, uint8_t payLength)
{
#	if _MONITOR>=1
	if (payLength>=128) {
		if ((debug>=2) && (pdebug & P_TX)) {
			mPrint("V sendPkt2:: len="+String(payLength));
		}
		return false;
	}
#	endif //_MONITOR
	...
	// MMM?
	writeRegister2(REG_FIFO_ADDR_PTR, (uint8_t) readRegister2(REG_FIFO_TX_BASE_AD));	// 0x0D, 0x0E
	writeRegister2(REG_PAYLOAD_LENGTH, (uint8_t) payLength);				// 0x22	

	writeBuffer2(REG_FIFO, (uint8_t *) payLoad, payLength);

#	if _MONITOR>=1
	if ((debug >=1) && (pdebug & P_TX)) {
		mPrint("sendPkt2:: payLength="+String(payLength)+", REG_FIFO=0x"+String(readRegister2(REG_FIFO),HEX) );
	}
#	endif
...
int loraWait2(struct LoraDown *LoraDown)
{
	if (LoraDown->imme == 1) {
		if ((debug>=1) && (pdebug & P_TX)) {
			mPrint("loraWait2:: imme is 1");
		}
		return(1);
	}
...
#		if _MONITOR>=1
		if (delayTmst > 8000000) {
			String response= "v loraWait2:: ERROR: ";
			printDwn(LoraDown, response);
			mPrint(response);
		}
		else {
			String response="v loraWait2:: return 0: ";
			printDwn(LoraDown, response);
			mPrint(response);
		}
#		endif //_MONITOR
...
//
//	1. opmode2 Lora (only in Sleep mode)
//	2. opmode2 StandBY
//	3. Configure Modem
//	4. Configure Channel
//	5. write PA Ramp
...
// 16. opmode2 TX, start actual transmission
//
// Transmission to the device not is not done often, but is time critical.
// ----------------------------------------------------------------------------------------

void txLoraModem2(struct LoraDown *LoraDown)
{
	_state2 = S_TX;
		
	// 1. Select LoRa modem from sleep mode
	//opmode2(OPMODE_SLEEP);											// set 0x01
	//opmode2(OPMODE_LORA);											// set 0x01 to 0x80
	
	// Assert the value of the current mode
	ASSERT((readRegister2(REG_OPMODE) & OPMODE_LORA) != 0);
	
	// 2. enter standby mode (required for FIFO loading))
	opmode2(OPMODE_STANDBY);											// set 0x01 to 0x01
	
	// 3. Init spreading factor and other Modem setting
	setRate2(LoraDown->sf, LoraDown->crc);
	
	// Frequency hop2ping
	//writeRegister2(REG_HOP_PERIOD, (uint8_t) 0x00);				// set 0x24 to 0x00 only for receivers
	
	// 4. Init Frequency, config channel
	setFreq2(LoraDown->freq);

	writeRegister2(REG_SYNC_WORD, (uint8_t) 0x34);					// set 0x39 to 0x34==LORA_MAC_PREAMBLE
	writeRegister2(REG_PREAMBLE_LSB, (uint8_t) LoraDown->prea & 0xFF);
	writeRegister2(REG_PARAMP, (readRegister2(REG_PARAMP) & 0xF0) | 0x08); // set 0x0A  ramp-up time 50 uSec
	writeRegister2(REG_PADAC_SX1276,  0x84); 						// set 0x4D (PADAC) to 0x84
	writeRegister2(REG_DET_TRESH, 0x0A);								// Detection Treshhold
	writeRegister2(REG_LNA, (uint8_t) LNA_MAX_GAIN);  				// set reg 0x0C to 0x23
	
	// 6. Set power level, REG_PAC
	setPow2(LoraDown->powe);
	
	// 7. prevent node to node communication
	//writeRegister2(REG_INVERTIQ, readRegister2(REG_INVERTIQ) | (uint8_t)(LoraDown->iiq));	// set 0x33, (0x27 up or |0x40 down)
	writeRegister2(REG_INVERTIQ, readRegister2(REG_INVERTIQ) | (uint8_t)(LoraDown->iiq)); // set 0x33 to (0x27 (reserved) | 0x40) for downstream
	
	// 8. set the IRQ mapping DIO0=TxDone DIO1=NOP DIO2=NOP (or less for 1ch gateway)
    writeRegister2(REG_DIO_MAPPING_1, (uint8_t)(
    ...
    writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) ~IRQ_LORA_TXDONE_MASK);
	
	// 10. clear all radio IRQ flags
    writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);
	//writeRegister2(REG_IRQ_FLAGS, (uint8_t) IRQ_LORA_TXDONE_MASK);	// set reg 0x12 to 0x08, clear TXDONE

	// FSTX setting
	opmode2(OPMODE_FSTX);											// set reg 0x01 to 0x02 (actual value becomes 0x82)
	delay(1);														// MMM

	//Set the base addres of the transmit buffer in FIFO
	writeRegister2(REG_FIFO_ADDR_PTR, (uint8_t) readRegister2(REG_FIFO_TX_BASE_AD));	// set 0x0D to 0x0F (contains 0x80);	

	//For TX we have to set the PAYLOAD_LENGTH
	writeRegister2(REG_PAYLOAD_LENGTH, (uint8_t) LoraDown->size);	// set reg 0x22 to  0x40==64Byte long

	//For TX we have to set the MAX_PAYLOAD_LENGTH
	writeRegister2(REG_MAX_PAYLOAD_LENGTH, (uint8_t) MAX_PAYLOAD_LENGTH);	// set reg 0x22, max 0x40==64Byte long

	// 11, 12, 13, 14. write the buffer to the FiFo
	sendPkt2(LoraDown->payLoad, LoraDown->size);
	
	// 16. Initiate actual transmission of FiFo, after opmode2 TX it switches to standby mode
	delay(5);														// MMM works
	opmode2(OPMODE_TX);												// set reg 0x01 to 0x03 (actual value becomes 0x83)
...
// 6. 
// 7. Set opmode2 to OPMODE_RX
// 8. Set _state2 to S_RX
...
void rxLoraModem2()
{
	...
	//opmode2(OPMODE_LORA);										// Is already so
	
	// 2. Put the radio in sleep mode
	opmode2(OPMODE_STANDBY);										// CAD set 0x01 to 0x00
	
	// 3. Set frequency based on value in freq
	setFreq2(freqs[gwayConfig.ch2].upFreq);						// set to the right frequency

	// 4. Set spreading Factor and CRC
    setRate2(sf, 0x04);
	
	// prevent node to node communication
	writeRegister2(REG_INVERTIQ, (uint8_t) 0x27);				// 0x33, 0x27; to reset from TX
	...
	//For TX we have to set the PAYLOAD_LENGTH
    //writeRegister2(REG_PAYLOAD_LENGTH, (uint8_t) PAYLOAD_LENGTH);	// set 0x22, 0x40==64Byte long

	// Set CRC Protection for MAX payload protection
	//writeRegister2(REG_MAX_PAYLOAD_LENGTH, (uint8_t) MAX_PAYLOAD_LENGTH);	// set 0x23 to 0x80==128
	
	//Set the start address for the FiFO (Which should be 0)
	writeRegister2(REG_FIFO_ADDR_PTR, (uint8_t) readRegister2(REG_FIFO_RX_BASE_AD));	// set 0x0D to 0x0F (contains 0x00);
	
	// Low Noise Amplifier used in receiver
	writeRegister2(REG_LNA, (uint8_t) LNA_MAX_GAIN);  			// 0x0C, 0x23
	
	// 5. Accept no interrupts except RXDONE, RXTOUT en RXCRC
	writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) ~(
		IRQ_LORA_RXDONE_MASK |
		IRQ_LORA_RXTOUT_MASK |
		IRQ_LORA_HEADER_MASK |
		IRQ_LORA_CRCERR_MASK));

	// set frequency hop2ping
	if (gwayConfig.hop) {
		//writeRegister2(REG_HOP_PERIOD, 0x01);					// 0x24, 0x01 was 0xFF
		writeRegister2(REG_HOP_PERIOD,0x00);						// 0x24, 0x00 was 0xFF
	}
	else {
		writeRegister2(REG_HOP_PERIOD,0x00);						// 0x24, 0x00 was 0xFF
	}
	...
	// 7+8.  Set the opmode2 to either single or continuous receive. The first is used when
	// every message can come on a different SF, the second when we have fixed SF
	if (gwayConfig.cad) {
		// cad Scanner setup, set _state2 to S_RX
		// Set Single Receive Mode, and go in STANDBY mode after receipt
		_state2= S_RX;											// 8.
		opmode2(OPMODE_RX_SINGLE);								// 7. 0x80 | 0x06 (listen one message)
	}
	else {
...
#		if _DUSB>=1
		if (gwayConfig.hop) {
			Serial.println(F("rxLoraModem2:: ERROR continuous receive in hop2 mode"));
		}
#		endif
		opmode2(OPMODE_RX);										// 7. 0x80 | 0x05 (listen)
	}
...
// The function sets the _state2 to S_SCAN
...
void cadScanner2()
{
	...
	// 2. Put the radio in sleep mode
	opmode2(OPMODE_STANDBY);										// Was old value
	...
	writeRegister2(REG_SYNC_WORD, (uint8_t) 0x34);				// set reg 0x39 to 0x34
	...
	//writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF );						// May work better, clear ALL interrupts
...
// Subsequent changes to the modem state etc. done by txLoraModem2 or rxLoraModem2
// After initialisation the modem is put in rx mode (listen)
//	1.	Set Radio to sleep
//	2.	Set opmode2 to LoRa
//	3.	Set Frequency
...
void initLoraModem2()
{
	...
	digitalWrite(pins.rst2, LOW);
	delayMicroseconds(10000);
    digitalWrite(pins.rst2, HIGH);
	delayMicroseconds(10000);
	digitalWrite(pins.ss2, HIGH);
#else
...
#endif
	// 1. Set radio to sleep
	opmode2(OPMODE_SLEEP);										// set reg 0x01 to 0x00

	// 2 Set LoRa Mode
	opmode2(OPMODE_LORA);										// set reg 0x01 to 0x80
	
	// 3. Set frequency based on value in gwayConfig.ch
	setFreq2(freqs[gwayConfig.ch2].upFreq);						// set to 868.1MHz or the last saved frequency
...
#	endif
    ...
    uint8_t version = readRegister2(REG_VERSION);				// read reg 0x34 the LoRa chip version id
...
#		if _DUSB>=1
			Serial.print(F("Unknown transceiver2="));
			Serial.print(version,HEX);
			Serial.print(F(", pins.rst2 ="));	Serial.print(pins.rst2);
			Serial.print(F(", pins.ss2  ="));	Serial.print(pins.ss2);
			Serial.print(F(", pins.dio02 ="));	Serial.print(pins.dio02);
			Serial.print(F(", pins.dio12 ="));	Serial.print(pins.dio12);
			Serial.print(F(", pins.dio22 ="));	Serial.print(pins.dio22);
			Serial.println();
			Serial.flush();
#		endif
		//die("initLoraModem2, unknown transceiver?");				// Maybe first try another kind of receiver
    }
	// If we are here, the chip is recognized successfully
	
	// 6. set sync word
	writeRegister2(REG_SYNC_WORD, (uint8_t) 0x34);				// set reg 0x39 to 0x34 LORA_MAC_PREAMBLE
	
	// prevent node to node communication
	writeRegister2(REG_INVERTIQ,0x27);							// set reg 0x33 to 0x27; reset from TX
	
	// Max Payload length is dependent on 256 byte buffer. At startup TX starts at
	// 0x80 and RX at 0x00. RX therefore maximized at 128 Bytes
	writeRegister2(REG_MAX_PAYLOAD_LENGTH,PAYLOAD_LENGTH);		// set reg 0x23 to 0x80==128 bytes
	//writeRegister2(REG_MAX_PAYLOAD_LENGTH,MAX_PAYLOAD_LENGTH);	// set reg 0x23 to 0x80==128 bytes
	writeRegister2(REG_PAYLOAD_LENGTH,PAYLOAD_LENGTH);			// set reg 0x22,to 0x40==64Byte long
	
	writeRegister2(REG_FIFO_ADDR_PTR, (uint8_t) readRegister2(REG_FIFO_RX_BASE_AD));	// set reg 0x0D to 0x0F
	writeRegister2(REG_HOP_PERIOD,0x00);							// set reg 0x24, to 0x00
	...
	writeRegister2(REG_PADAC_SX1276,  0x84); 					// set reg 0x4D (PADAC) to 0x84
	//writeRegister2(REG_PADAC_SX1276, readRegister2(REG_PADAC_SX1276) | 0x4);

	// Set treshold according to sx1276 specs
	writeRegister2(REG_DET_TRESH, 0x0A);
...
// ----------------------------------------------------------------------------------------
// Void function startReceiver2.
// This function starts the receiver loop of the LoRa service.
// It starts the LoRa modem with initLoraModem2(), and then starts
// the receiver either in single message (CAD) of in continuous
// reception (STD).
// ----------------------------------------------------------------------------------------
void startReceiver2() 
{
	initLoraModem2();								// XXX 180326, after adapting this function 
	if (gwayConfig.cad) {
#		if _DUSB>=1
		if ((debug>=1) && (pdebug & P_SCAN)) {
			Serial.println(F("S PULL:: _state2 set to S_SCAN"));
...
#		endif
	...
	writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);
	writeRegister2(REG_IRQ_FLAGS, 0xFF);				// Reset all interrupt flags
}
...
// ----------------------------------------------------------------------------------------
void ICACHE_RAM_ATTR Interrupt_02()
{
	_event2=1;
}
