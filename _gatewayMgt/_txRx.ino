...
//	buff_index:
// ----------------------------------------------------------------------------
int buildPacket(uint8_t *buff_up, struct LoraUp *LoraUp, bool internal, uint8_t cc_select) 
{
...
uint8_t selec_chip = cc_select;
	...
	}else if(selec_chip==2){
		freqs[gwayConfig.ch2].jsonfreq.toCharArray(cfreq, 13);
	}
	if ((LoraUp->sf<6) || (LoraUp->sf>12)) { 							// Lora datarate & bandwidth SF6-SF12, 16-19 useful chars */
		LoraUp->sf=7;
	}
	if ((LoraUp->sf2<6) || (LoraUp->sf2>12)) { 							// Lora datarate & bandwidth SF6-SF12, 16-19 useful chars */
		LoraUp->sf2=7;
	}			
...
//		year(),month(),day(),hour(),minute(),second());
	...
	if(selec_chip==1){
		...
		buff_index += snprintf((char *)(buff_up + buff_index), RX_BUFF_SIZE-buff_index
		, ",\"datr\":\"SF%uBW%u\",\"codr\":\"4/5\",\"lsnr\":%li,\"rssi\":%d,\"size\":%u,\"data\":\""
		, LoraUp->sf2, freqs[gwayConfig.ch2].upBW, (long)SNR, prssi-rssicorr, messageLength);
	}
...
// ----------------------------------------------------------------------------
int receivePacket(uint8_t selec_chip)
{
	uint8_t ss_chip_sel = selec_chip;
...
#			endif //_PROFILER
            ...
            int build_index = buildPacket(buff_up, &LoraUp, false, ss_chip_sel);
