...
//
// _event2 is the software interrupt: If set this function is executed from loop(),
// the function should itself take care of setting or resetting the variable.
//
// STATE MACHINE
// The program uses the following state machine (in _state2), all states
...
//
// So _state2 has one of the following state values:
//
...
// If we receive an interrupt that does not belong to a _state2 then print error.
// _event2 is a special variable which indicate that an interrupt event has happened
...
void stateMachine2()
{
	...
	//
	uint8_t flags = readRegister2(REG_IRQ_FLAGS);
	uint8_t mask  = readRegister2(REG_IRQ_FLAGS_MASK);
	uint8_t intr  = flags & ( ~ mask );							// Only react on non masked interrupts
	uint8_t rssi;
	_event2=0;													// Reset the interrupt detector	
...
#	endif //_MONITOR
	...
	if ((gwayConfig.hop) && (intr == 0x00))
	{
		...
		//
		if ((_state2 == S_SCAN) || (_state2 == S_CAD)) {
...
#					endif //_MONITOR
			...
			if (((micros() - doneTime) > doneWait) &&
				((_state2 == S_SCAN) || (_state2 == S_CAD)))
			{
				_state2 = S_SCAN;
				hop2();											// increment gwayConfig.ch = (gwayConfig.ch + 1) % NUM_HOPS ;
				cadScanner2();									// Reset to initial SF, leave frequency "freqs[gwayConfig.ch]"
#				if _MONITOR>=1
...
#				endif //_MONITOR
			...
			// If timeout occurs and still no _event2, then hop2
			// and start scanning again
			//
			if ((micros() - eventTime) > eventWait ) 
			{
				_state2 = S_SCAN;
				hop2();											// gwayConfig.ch= (gwayConfig.ch+1)%NUM_HOPS ;
				cadScanner2();									// Reset to initial SF, leave "freqs[gwayConfig.ch]"
#				if _MONITOR>=1
...
#			endif //_MONITOR
	...
	// For hop2 situations we do not get interrupts, so we have to
...
#		endif //_MONITOR
		...
		writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF );				// Clear ALL interrupts
		writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00 );			// Accept ALL interrupts
		_event2=0;
	  break;
	  ...
	  case S_SCAN:
		...
		if (intr & IRQ_LORA_CDDETD_MASK) {
			...
			_state2 = S_RX;											// Set state to receiving

			// Set RXDONE interrupt to dio0, RXTOUT to dio1
			writeRegister2(REG_DIO_MAPPING_1, (
			...
			//if (gwayConfig.cad) 									// XXX 180520 make sure we start reading asap in hop2
			//	delayMicroseconds( RSSI_WAIT );						// Wait some microseconds less

			rssi = readRegister2(REG_RSSI);							// Read the RSSI
			_rssi2 = rssi;											// Read the RSSI in the state variable

			_event2=0;												// Make 0, as soon as we have an interrupt
			detTime2=micros();										// mark time that preamble detected
...
#			endif //_MONITOR
			...
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);			// reset all interrupt flags
			opmode2(OPMODE_RX_SINGLE);								// set reg 0x01 to 0x06 for receiving
...
#				endif //_MONITOR
				_state2 = S_CAD;										// promote to next level
				_event2=0;
			}
...
#				endif //_MONITOR
		...
		// CDDECT or CDDONE. it is probably soft interrupt _event2==1
		// So if gwayConfig.hop2 we change the frequency and restart the
		// interrupt in order to check for CDONE on other frequencies
		// if gwayConfig.hop2 we start at the next frequency, hop2 () sets the sf to SF7.
		...
		// We do this by changing _event2 to 1 in loop() only for gwayConfig.hop2 and
		// use _event2=0 for non hop2.
		//
		else if (intr == 0x00) 
		{
			_event2=0;												// XXX 26/12/2017 !!! NEED
		}
...
#			endif //_MONITOR
			_state2=S_SCAN;
			//_event2=1;												// XXX 19/06/03 loop until interrupt
			writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);
		}
	  ...
	  case S_CAD:
		...
		if (intr & IRQ_LORA_CDDETD_MASK) {
			...
			//writeRegister2(REG_IRQ_FLAGS, IRQ_LORA_CDDETD_MASK | IRQ_LORA_RXDONE_MASK);

			// If we want to reset CRC, HEADER and RXTOUT flags as well
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF );			// XXX 180326, reset all CAD Detect interrupt flags

			//_state2 = S_RX;										// XXX 180521 Set state to start receiving
			opmode2(OPMODE_RX_SINGLE);								// set reg 0x01 to 0x06, initiate READ
...
#			endif //_MONITOR
			_state2 = S_RX;											// Set state to start receiving
		...
		//
			...
			//
			if (((uint8_t)sf) < freqs[gwayConfig.ch2].upHi) {

				sf = (sf_t)((uint8_t)sf+1);							// Increment sf
				setRate2(sf, 0x04);									// Set SF with CRC==on

				opmode2(OPMODE_CAD);									// Scanning mode
...
#				if _MONITOR>=1
				...
				_event2=0;											// XXX 180324, when increasing SF loop, ws 0x00
				writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);	// Accept all
				//writeRegister2(REG_IRQ_FLAGS, IRQ_LORA_CDDONE_MASK | IRQ_LORA_CDDETD_MASK);
				writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF );		// Clear all
...
#				endif //_MONITOR
			...
			//
				...
				// Reset Interrupts
				_event2=1;											// reset soft intr, to state machine again
				writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);	// Reset the interrupt mask
				writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF );		// or IRQ_LORA_CDDONE_MASK

				_state2 = S_SCAN;									// As soon as we reach SF12 do something
				sf = (sf_t) freqs[gwayConfig.ch2].upLo;
				cadScanner2();										// Which will reset SF to lowest SF
...
#				endif //_MONITOR
		...
		// This means _event2 was set but there was no real interrupt (yet).
		// So we clear _event2 and wait for next (soft) interrupt.
...
#			endif //_MONITOR
			//_event2=1;												// Stay in CAD _state2 until real interrupt
		}

		// else we do not recognize the interrupt. We print an error
		// and restart scanning. If hop2 we start at gwayConfig.ch==0
		//
		else {
#			if _MONITOR>=1
			if ( debug>=0) { 
...
#			endif //_MONITOR
			_state2 = S_SCAN;
			sf = SF7;
			cadScanner2();											// Scan and set SF7
			
			// Reset Interrupts
			_event2=1;												// If unknown interrupt, restarts
			writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);		// Reset the interrupt mask
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);			// Reset all interrupts
...
#			endif	
			...
			// - print Error message
			// - Set _state2 to SCAN
			// - Set _event2=1 so that we loop until we have an interrupt
			// - Reset the interrupts
			// - break
			// NOTE: receivePacket also increases .ok0 - .ok2 counter

			if((LoraUp.size = receivePkt2(LoraUp.payLoad)) <= 0) {
#				if _MONITOR>=1
				if ((debug>=0) && (pdebug & P_RX)) {
...
#			if _MONITOR>=1
			if ((debug >=2) && (pdebug & P_RX)) {
				...
				response += String(rxDoneTime - detTime2);
				mStat(intr, response);
				mPrint(response);
			}
#			endif //_MONITOR
			...
			uint8_t value = readRegister2(REG_PKT_SNR_VALUE);		// 0x19; 
			...
			// Packet RSSI
			LoraUp.prssi = readRegister2(REG_PKT_RSSI);				// read register 0x1A, packet rssi
			...
			LoraUp.sf2 = readRegister2(REG_MODEM_CONFIG2) >> 4;
//********************************************************************************************************
			// If read was successful, read the package from the LoRa bus
			//
			uint8_t ss_chip_selec = 2;
			if (receivePacket(ss_chip_selec) <= 0) {								// read is not successful
#				if _MONITOR>=1
				if ((debug>=0) && (pdebug & P_RX)) {
...
#				endif //_MONITOR
			...
			writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);			// Reset the interrupt mask
		...
		// This is the most common event in hop2 mode, possibly due to the fact
		...
		//
			...
			_event2=0;												// Is set by interrupt handlers
			writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00 );
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);			// reset all interrupts
...
#				endif //_MONITOR
				sf = SF7;
				cadScanner2();										// Start the scanner after RXTOUT
				_state2 = S_SCAN;									// New state is scan
			}

			// If not in cad mode we are in single channel single sf mode.
			//
			else {
				_state2 = S_RX;										// Receive when interrupted
				rxLoraModem2();
			}
			...
			//writeRegister2(REG_IRQ_FLAGS, IRQ_LORA_HEADER_MASK);
#			if _MONITOR>=1
...
#			endif //_MONITOR
	  ...
	  // It therefore is no use to set _event2==1 as transmission might
...
#			endif //_MONITOR
		...
		_state2=S_TXDONE;
		_event2=1;													// Or remove the break below
...
#		endif //_MONITOR
	  ...
	  // was successful.
	  // If we receive an interrupt on dio0 _state2==S_TX it is a TxDone interrupt
...
#			endif //_MONITOR
			...
			if ((gwayConfig.cad) || (gwayConfig.hop)) {				// XXX 26/02
				...
				_state2 = S_SCAN;
				sf = SF7;
				cadScanner2();										// Start the scanner after TX cycle
			}
			else {
				_state2 = S_RX;
				rxLoraModem2();		
...
#				endif //_MONITOR
		...
		// If a soft _event2==0 interrupt and no transmission finished:
		else if ( intr != 0 ) {
#			if _MONITOR>=1
			if ((debug>=0) && (pdebug & P_TX)) {
...
#			endif
			writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);
			writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);			// reset interrupt flags
			_event2=0;
			_state2=S_SCAN;
		}
...
#				endif //_MONITOR
			...
			if (( _state2 == S_TXDONE ) && (( micros() - sendTime) > 8000000 )) {
#				if _MONITOR>=1
				if ((debug>=1) && (pdebug & P_TX)) {
...
#		endif //_MONITOR
		...
		//writeRegister2(REG_IRQ_FLAGS_MASK, (uint8_t) 0x00);		// receive all
		//writeRegister2(REG_IRQ_FLAGS, (uint8_t) 0xFF);				// reset interrupt flags
...
#		if _MONITOR>=1
		if ((debug>=0) && (pdebug & P_PRE)) {
			mPrint("ERR state=" + String(_state2));	
		}
#		endif //_MONITOR
...
#			if _MONITOR>=1
			if (debug>=0) {
				String response = "default:: Unknown _state2 ";
				mStat(intr, response);
				mPrint(response);
			}
#			endif //_MONITOR
			_state2 = S_SCAN;
			sf = SF7;
			cadScanner2();											// Restart the state machine
			_event2=0;									
		}
